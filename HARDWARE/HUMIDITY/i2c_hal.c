...
//==============================================================================
  ...
  IIC_SCL = 1;
  delay_us(1*hum_read_delay);//DelayMicroSeconds(1);
  IIC_SDA = 0;
  delay_us(10*hum_read_delay);//DelayMicroSeconds(10);  // hold time start condition (t_HD;STA)
  IIC_SCL = 0;
  delay_us(10*hum_read_delay);//DelayMicroSeconds(10);
}
...
//==============================================================================
  ...
  delay_us(1*hum_read_delay);
  IIC_SCL = 1;
  delay_us(10*hum_read_delay);  // set-up time stop condition (t_SU;STO)
  IIC_SDA = 1;
  delay_us(10*hum_read_delay);
}
...
//==============================================================================
  ...
  {
    if((mask & txByte) == 0) IIC_SDA = 0; // masking txByte, write bit to SDA-Line
    else                     IIC_SDA = 1;
    delay_us(5*hum_read_delay);               // data set-up time (t_SU;DAT)
    IIC_SCL = 1;                         // generate clock pulse on SCL
    delay_us(5*hum_read_delay);               // SCL high time (t_HIGH)
    IIC_SCL = 0;
...
//==============================================================================
  ...
  *rxByte = 0x00;
  ...
  { 
	...
	SDA_OUT();
  if(ack == ACK) IIC_SDA = 0;              // send acknowledge if necessary
  else           IIC_SDA = 1;
  delay_us(1*hum_read_delay);                  // data set-up time (t_SU;DAT)
  IIC_SCL = 1;                            // clk #9 for ack
  delay_us(5*hum_read_delay);                  // SCL high time (t_HIGH)
  IIC_SCL = 0;
  IIC_SDA = 1;                            // release SDA-line
  delay_us(20*hum_read_delay);                 // wait to see byte package on scope
